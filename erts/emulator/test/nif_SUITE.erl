...
-export([all/1, fin_per_testcase/2, basic/1, reload/1, upgrade/1, heap_frag/1,
	 ...
	 threading/1, neg/1, length_test/3, get_length/1, check_is/9, is_checks/1]).
...
all(suite) ->
    [basic, reload, upgrade, heap_frag, types, many_args, binaries, get_string,
     ...
     resource_takeover, threading, neg, get_length, is_checks].
...
get_length(doc) -> ["Test all enif_get_length API functions"];
get_length(Config) when is_list(Config) ->
    ?line ensure_lib_loaded(Config, 1),
    ?line ok = length_test(foobar, "hejsan", [$h,"ejs",<<"an">>]).

is_checks(doc) -> ["Test all enif_is API functions"];
is_checks(Config) when is_list(Config) ->
    ?line ensure_lib_loaded(Config, 1),
    ?line ok = check_is(foobar, <<19,98>>, make_ref(), ok, fun() -> ok end,
			self(), hd(erlang:ports()), [],
			{foobar, "hejsan", [$h,"ejs",<<"an">>]}).
...
length_test(_,_,_) -> ?nif_stub.
check_is(_,_,_,_,_,_,_,_,_) -> ?nif_stub.
